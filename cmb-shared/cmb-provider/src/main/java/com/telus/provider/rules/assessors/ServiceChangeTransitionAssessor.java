...
public class ServiceChangeTransitionAssessor extends ConditionAssessor {
	
	public ServiceChangeTransitionAssessor(String conditionId) {
		super(conditionId);
	}
	...
	public ConditionResult evaluate(WorkingMemory workingMemory) throws TelusAPIException {		
		...
		WorkingMemoryElement serviceChangeTransitionElement = (WorkingMemoryElement)workingMemory.getAttribute(
				RuleConstants.ATTRIBUTE_SERVICE_TRANSITION);
		...
		if (oldResult != null && !serviceChangeTransitionElement.isModified()) {
		...
		String serviceChangeTransition = (String)serviceChangeTransitionElement.getElement();

		ConditionResult result = new ConditionResult(RuleConstants.CONDITION_TYPE_TEXT, serviceChangeTransition);
