...
public class BanHasOnePoolingGroupSubLeftAssessor extends ConditionAssessor {

	public BanHasOnePoolingGroupSubLeftAssessor(String conditionId) {
		super(conditionId);
	}
	...
	public ConditionResult evaluate(WorkingMemory workingMemory) throws TelusAPIException {		
		...
		if (result.getTextResult().equals(RuleConstants.CONDITION_VALUE_BOOLEAN_TRUE))
			...
			result = new ConditionResult(RuleConstants.CONDITION_TYPE_BOOLEAN, 
					Boolean.toString(hasOnePoolingSubLeft(poolingGroupId.intValue(), account, subscriber, includeReserved.booleanValue())));
	...
	private boolean hasOnePoolingSubLeft(int poolingGroupId, Account account, Subscriber subscriber, boolean includeReserved) 
	throws TelusAPIException {
		...
		int otherPoolingSubsLeft = 0;

		// loop through all subscribers
		for (int i = 0; i < (subscriberCounts != null ? subscriberCounts.length : 0); i++) {

			// loop through all active and / or reserved subscribers
			String[] activeSubs;
			if (includeReserved)
			...
			for (int j = 0; j < (activeSubs != null ? activeSubs.length : 0); j++) {
				if (activeSubs[j] != null && activeSubs[j].trim().length() > 0 && !(activeSubs[j].equals(subscriberId)))
					otherPoolingSubsLeft++;
			}

			// if we have zero or just one subscriber so far, we'll need to loop through the suspended subscribers as well
			if (otherPoolingSubsLeft < 2) {
				...
				for (int k = 0; k < (suspendedSubs != null ? suspendedSubs.length : 0); k++) {
					if (suspendedSubs[k] != null && suspendedSubs[k].trim().length() > 0 && !(suspendedSubs[k].equals(subscriberId))) {
						...
						if (sub != null	&& (sub.getActivityReasonCode().equalsIgnoreCase(RuleConstants.ACTIVITY_REASON_SUSPEND_LOST) 
							...
							otherPoolingSubsLeft++;
							// check the number of subs again to limit the number of subscriber retrievals
							if (otherPoolingSubsLeft > 1)
								break;
						}
					}
				}
