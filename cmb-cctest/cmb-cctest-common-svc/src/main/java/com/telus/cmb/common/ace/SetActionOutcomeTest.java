...
public class SetActionOutcomeTest {
	...
	public void test() throws Throwable {
		...
		SetActionOutcomeImpl outcomeImpl = new SetActionOutcomeImpl(getService());
		GetActionListImpl getActionImpl = new GetActionListImpl(getService());
		try {
			List<AceAdvisorActionInfo> actionList = getActionImpl.getActionList(phoneNumber, channelType, channelAttributeMap);
			for (AceAdvisorActionInfo action : actionList) {
				outcomeImpl.acceptOffer(phoneNumber, channelType, action.getActionName());
			}
		} catch (Exception e) {
