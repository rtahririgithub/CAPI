...
@UseTestDataFrom(value= "cmb/regression/activation/testdata-PostpaidCorporateSubscriberActivation-$DATADIR.csv")
@WithTagValuesOf({"epic: Regression", "test type: web service", "test style: structured (recommended)"})
public class CorporateSubscriberActivationTest extends WebServiceBaseTest {
	...
	public String pricePlan, services, marketAreaCode;																						//Activation
	...
	@Test
	public void testSubscriberActivationForCorporateAccounts() throws Exception {
		reservePhoneNumber();
		activateSubscriber();
		cancelSubscriber();
	}
	...
	@Test
	public void testReservePhoneNumberForCorporateAccounts() throws Exception {		
		reservePhoneNumber();
		releasePhoneNumber();
	}
	...
	private void activateSubscriber() throws Exception {
		activationSteps.activateSubscriber(SMSHelper.createDefaultActivateSubscriber(ban, subscriberId, phoneNumber, marketAreaCode, productType, activationSteps.getLogicalDate(), 
				activationSteps.getAvailableUSimEquipment(DefaultValues.BRAND_ID_TELUS), pricePlan, services));
