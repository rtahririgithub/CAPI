...
public class IneligiblePaymentArrangementExtensionException extends
        TelusAPIException {
    ...
    public static final int PREVIOUSLY_DISHONOURED_SELFSERVE_PAYMENT_ARRANGEMENTS = REASON_OFFSET + 11;

    //added
    public static final int INSUFFICIENT_TIME_BEFORE_NEXT_INVOICE_DUE_DATE = REASON_OFFSET + 12;
    public static final int ACCOUNT_HAS_BEEN_OVERTURNED_TO_AGENCY = REASON_OFFSET + 13;
    
    /** @deprecated this constant is no longer used anywhere in the code */
    public static final int NEXT_STEP_DATE_EXCEEDS_POSTING_DATE = REASON_OFFSET + 14;
    ...
    public IneligiblePaymentArrangementExtensionException(int reason,Account account) {
    ...
    static {
        ...
        reasons.put(new Integer(UNSUPPORTED_CREDIT_CLASS), "No automated payment arrangement is allowed for this credit class.");
        reasons.put(new Integer(ACCOUNT_HAS_AMOUNTS_IN_90DAYS_BUCKET), "Overdue balance aging exceeds allowable limit.");
        reasons.put(new Integer(PREVIOUSLY_DISHONOURED_SELFSERVE_PAYMENT_ARRANGEMENTS), "A dishonoured payment arrangement currently exists on this account.");
        reasons.put(new Integer(INSUFFICIENT_TIME_BEFORE_NEXT_INVOICE_DUE_DATE), "Insufficient time before the next due date.  No extension will be granted.");
        reasons.put(new Integer(ACCOUNT_HAS_BEEN_OVERTURNED_TO_AGENCY), "Account has been referred to an agency for collection.");
        reasons.put(new Integer(NEXT_STEP_DATE_EXCEEDS_POSTING_DATE), "No Payment arrangment is currently necessary, the next step date exceeds the option posting date.");
