...
@WebService(portName = "WirelessAccountLifecycleMgmtServicePort", serviceName = "WirelessAccountLifecycleMgmtService_v1_2", targetNamespace = "http://telus.com/wsdl/CMO/OrderMgmt/WirelessAccountLifecycleMgmtService_1", wsdlLocation = "/wsdls/WirelessAccountLifecycleMgmtService_v1_2.wsdl", endpointInterface = "com.telus.cmb.jws.WirelessAccountLifecycleMgmtServicePort")
@BindingType("http://schemas.xmlsoap.org/wsdl/soap/http")
@SchemaValidation(handler = com.telus.cmb.jws.ServiceSchemaValidator.class)
public class WirelessAccountLifecycleMgmtService_12 extends BaseService implements WirelessAccountLifecycleMgmtServicePort {

	@Override
	@ServiceBusinessOperation(errorCode = "CMB_WALM_0001", errorMessage = "Create Postpaid Account")
	public CreatePostpaidAccountResponse createPostpaidAccount(final CreatePostpaidAccount parameters, final OriginatingUserType createPostpaidAccountInSoapHdr) throws PolicyException, ServiceException {
		...
		return execute(new ServiceInvocationCallback<CreatePostpaidAccountResponse>() {
			...
			public CreatePostpaidAccountResponse doInInvocationCallback(ServiceInvocationContext context) throws Throwable {
				...
				PostpaidAccountCreationResponseInfo postpaidAccountCreationResponse = getAccountLifecycleFacade(context).createPostpaidAccount(accountInfo, customerId, creditIdentityInfos,
						selectedBusinessCreditIdentity, auditHeaderInfo, auditInfo, context.getAccountLifeCycleFacadeSessionId());
	...
	public PaymentNotificationResponseType applyPaymentNotification(final String billingAccountNumber, final String paymentAmount,
		...
		return execute(new ServiceInvocationCallback<PaymentNotificationResponseType>() {

			@SuppressWarnings("unchecked")
			@Override
			public PaymentNotificationResponseType doInInvocationCallback(ServiceInvocationContext context) throws Throwable {
				
				// Create the response
				PaymentNotificationResponseType response = new PaymentNotificationResponseType();

				// Map auditing info
				AuditInfo domainAuditInfo = auditInfo != null ? getAuditInfoMapper().mapToDomain(auditInfo) : null;
				
				// Make the call to the AccountLifecycleFacade EJB
				PaymentNotificationResponseInfo paymentNotificationResponseInfo = getAccountLifecycleFacade(context).applyPaymentNotification(Integer.valueOf(billingAccountNumber), Double.valueOf(paymentAmount), 
						notificationSuppressionInd, domainAuditInfo, context.getAccountLifeCycleFacadeSessionId());

				// Map the result to the response
				response.setId(paymentNotificationResponseInfo.getId());
				response.setDueDate(paymentNotificationResponseInfo.getPaymentNotificationDueDate());
				response.setMemo(getMemoMapper_40().mapToSchema(paymentNotificationResponseInfo.getMemo()));
				response.setFollowUp(getFollowUpMapper().mapToSchema(paymentNotificationResponseInfo.getFollowUp()));
				response.getActivityResultList().addAll(getActivityResultMapper().mapToSchema(paymentNotificationResponseInfo.getActivityResults()));
	...
	public PaymentArrangementEligibilityResponseType checkPaymentArrangementEligibility(final String billingAccountNumber, 
		...
		return execute(new ServiceInvocationCallback<PaymentArrangementEligibilityResponseType>() {

			@Override
			public PaymentArrangementEligibilityResponseType doInInvocationCallback(ServiceInvocationContext context) throws Throwable {

				// Create the response
				PaymentArrangementEligibilityResponseType response = new PaymentArrangementEligibilityResponseType();
				
				// Map auditing info
				AuditInfo domainAuditInfo = auditInfo != null ? getAuditInfoMapper().mapToDomain(auditInfo) : null;
				
				// Make the call to the AccountLifecycleFacade EJB
				PaymentArrangementEligibilityResponseInfo paymentArrangementEligibilityResponseInfo = getAccountLifecycleFacade(context).checkPaymentArrangementEligibility(Integer.valueOf(billingAccountNumber), 
						notificationSuppressionInd, domainAuditInfo, context.getAccountLifeCycleFacadeSessionId());
				
				// Map the result to the response
				response.setEligibilityInd(paymentArrangementEligibilityResponseInfo.isEligible());
				response.setBillingAccountNumber(String.valueOf(paymentArrangementEligibilityResponseInfo.getBanId()));
				response.setNextBillDate(paymentArrangementEligibilityResponseInfo.getNextBillDate());
				response.setPaymentArrangementDate(paymentArrangementEligibilityResponseInfo.getPaymentArrangementDate());
	...
	public PaymentArrangementResponseType applyPaymentArrangement(final String paymentAmount, final PaymentArrangementEligibilityResponseType paymentArrangementEligibilityResponse,
		...
		return execute(new ServiceInvocationCallback<PaymentArrangementResponseType>() {

			@SuppressWarnings("unchecked")
			@Override
			public PaymentArrangementResponseType doInInvocationCallback(ServiceInvocationContext context) throws Throwable {

				// Create the response
				PaymentArrangementResponseType response = new PaymentArrangementResponseType();
				
				// Map auditing info
				AuditInfo domainAuditInfo = auditInfo != null ? getAuditInfoMapper().mapToDomain(auditInfo) : null;
				
				// Make the call to the AccountLifecycleFacade EJB
				List<ActivityResultInfo> activityResultList = getAccountLifecycleFacade(context).applyPaymentArrangement(Integer.valueOf(paymentArrangementEligibilityResponse.getBillingAccountNumber()),
						Double.valueOf(paymentAmount), paymentArrangementEligibilityResponse.getNextBillDate(), paymentArrangementEligibilityResponse.getPaymentArrangementDate(), notificationSuppressionInd, 
						domainAuditInfo, context.getAccountLifeCycleFacadeSessionId());
				
				// Map the result to the response
				response.getActivityResultList().addAll(getActivityResultMapper().mapToSchema(activityResultList));
