...
@WebService(portName = "AccountInformationServicePort", serviceName = "AccountInformationService_v3_4", 
		targetNamespace = "http://telus.com/wsdl/CMO/InformationMgmt/AccountInformationService_3", 
		wsdlLocation = "/wsdls/AccountInformationService_v3_4.wsdl", 
		endpointInterface = "com.telus.cmb.jws.AccountInformationServicePort")
		@BindingType("http://schemas.xmlsoap.org/wsdl/soap/http")
public class AccountInformationService_34 extends BaseService implements AccountInformationServicePort {
	private static final int RETURN_MAXIMUM = 1000;
	private static final CreditCardMapper ccMapper = new CreditCardMapper();

	public AccountInformationService_34() {
	...
	public Account getAccountByAccountNumber(final String ban) throws PolicyException, ServiceException {
		...
		return execute( new ServiceInvocationCallback<Account>() {
			...
			public Account doInInvocationCallback(ServiceInvocationContext context) throws Throwable {
				...
				AccountInfo	accountInfo = accountInformationHelper.retrieveAccountByBan(banNumber);
				accountInfo.getCreditCheckResult0().setDeposits(accountInformationHelper.retrieveDepositsByBan(banNumber));
				accountInfo.setProductSubscriberLists(accountInformationHelper.retrieveProductSubscriberLists(banNumber));
	...
	public Account getAccountByPhoneNumber(final String phoneNumber) throws PolicyException, ServiceException {
		return execute(new ServiceInvocationCallback<Account>() {
			...
			public Account doInInvocationCallback(ServiceInvocationContext context) throws Throwable {
				AccountInformationHelper accountInformationHelper = getAccountInformationHelper(context);
				AccountInfo	accountInfo = accountInformationHelper.retrieveAccountByPhoneNumber(phoneNumber);
				accountInfo.getCreditCheckResult0().setDeposits(accountInformationHelper.retrieveDepositsByBan(accountInfo.getBanId()));
				accountInfo.setProductSubscriberLists(accountInformationHelper.retrieveProductSubscriberLists(accountInfo.getBanId()));
				return getAccountMapper().mapToSchema(accountInfo);
			}
		});
	}
