...
import com.telus.cmb.jws.MaxVoipLineBaseType;
import com.telus.cmb.jws.MaxVoipLineType;
...
public class AccountInformationServiceMapper {
	...
	public static class AccountMapper extends AbstractSchemaMapper<Account, AccountInfo> {
		...
		protected Account performSchemaMapping(AccountInfo source, Account target) {
			...
			} else if (source.isPostpaidCorporatePersonal() && source instanceof PostpaidCorporatePersonalAccountInfo) {
				target.setName(ConsumerNameMapper.getInstance().mapToSchema(((PostpaidCorporatePersonalAccountInfo) source).getName0()));
			} else if (source.isPrepaidConsumer() && source instanceof PrepaidConsumerAccountInfo) {
			...
			if (source.isPostpaidConsumer() && source instanceof PostpaidConsumerAccountInfo) {
				...
				PersonalCreditInformation creditInformation = mapPersonalCreditInformation(creditInfo, source.getAddress0());
				target.setPersonalCreditInformation(creditInformation);
			} else if (source.isPostpaidBusinessPersonal() && source instanceof PostpaidBusinessPersonalAccountInfo) {
...
//				target.setBanId(source.getBan());
	...
	private static class PrepaidAccountInfoMapper extends AbstractSchemaMapper<PrepaidAccountInfo, PrepaidConsumerAccountInfo> {
		...
		protected PrepaidAccountInfo performSchemaMapping(PrepaidConsumerAccountInfo source, PrepaidAccountInfo target) {
			...
			target.setSwipedAmount(source.getSwipedAmount());
			target.setInTrustBalanceExpiryDate(source.getInTrustBalanceExpiryDate());
			...
			target.setBalanceCapOrThresholdCode(source.getBalanceCapOrThresholdCode());
			target.setUsLongDistanceRate(source.getUSLongDistanceRate());
	...
	public static FollowUpRequestMapper FollowUpRequestMapper() {
		return FollowUpRequestMapper.getInstance();
	}
	
	public static class FollowUpRequestMapper extends AbstractSchemaMapper<FollowUpRequest, FollowUpInfo> {
		
		private static FollowUpRequestMapper INSTANCE;
		
		public FollowUpRequestMapper() {
			super(FollowUpRequest.class, FollowUpInfo.class);
		}

		protected static synchronized FollowUpRequestMapper getInstance() {
			if (INSTANCE == null) {
				INSTANCE = new FollowUpRequestMapper();
			}
			return INSTANCE;
		}
		
		@Override
		protected FollowUpInfo performDomainMapping(FollowUpRequest source, FollowUpInfo target) {

			target.setAssignedToWorkPositionId(source.getAssignedToWorkPositionId());
			target.setDueDate(source.getDueDate());
			target.setText(source.getFollowUpText());
			target.setType(source.getFollowUpType());

			return super.performDomainMapping(source, target);
		}

		@Override
		protected FollowUpRequest performSchemaMapping(FollowUpInfo source, FollowUpRequest target) {
			
			target.setAssignedToWorkPositionId(source.getAssignedToWorkPositionId());
			target.setDueDate(source.getDueDate());
			target.setFollowUpText(source.getText());
			target.setFollowUpType(source.getType());
	...
	public static CustomerNotificationPreferenceMapper CustomerNotificationPreferenceMapper() {
		return CustomerNotificationPreferenceMapper.getInstance();
	}

	public static class CustomerNotificationPreferenceMapper extends AbstractSchemaMapper<CustomerNotificationPreferenceType, CustomerNotificationPreferenceInfo> {

		private static CustomerNotificationPreferenceMapper INSTANCE;
		
		public CustomerNotificationPreferenceMapper() {
			super(CustomerNotificationPreferenceType.class, CustomerNotificationPreferenceInfo.class);
		}

		protected static synchronized CustomerNotificationPreferenceMapper getInstance() {
			if (INSTANCE == null) {
				INSTANCE = new CustomerNotificationPreferenceMapper();
			}
			return INSTANCE;
		}
		
		@Override
		protected CustomerNotificationPreferenceInfo performDomainMapping(CustomerNotificationPreferenceType source, CustomerNotificationPreferenceInfo target) {
		...
		@Override
		protected CustomerNotificationPreferenceType performSchemaMapping(CustomerNotificationPreferenceInfo source, CustomerNotificationPreferenceType target) {
	...
	public static CustomerNotificationPreferenceUpdateMapper CustomerNotificationPreferenceUpdateMapper() {
		return CustomerNotificationPreferenceUpdateMapper.getInstance();
	}

	public static class CustomerNotificationPreferenceUpdateMapper extends AbstractSchemaMapper<CustomerNotificationPreferenceUpdateType, CustomerNotificationPreferenceInfo> {
		
		private static CustomerNotificationPreferenceUpdateMapper INSTANCE;
		
		public CustomerNotificationPreferenceUpdateMapper() {
			super(CustomerNotificationPreferenceUpdateType.class, CustomerNotificationPreferenceInfo.class);
		}

		protected static synchronized CustomerNotificationPreferenceUpdateMapper getInstance() {
			if (INSTANCE == null) {
				INSTANCE = new CustomerNotificationPreferenceUpdateMapper();
			}
			return INSTANCE;
		}
		
		@Override
		protected CustomerNotificationPreferenceInfo performDomainMapping(CustomerNotificationPreferenceUpdateType source, CustomerNotificationPreferenceInfo target) {

			target.setCode(source.getCode());
			target.setDescription(source.getDescription());
			target.setUpdateModeCode(source.getUpdateModeCode());

			return super.performDomainMapping(source, target);
		}

		@Override
		protected CustomerNotificationPreferenceUpdateType performSchemaMapping(CustomerNotificationPreferenceInfo source, CustomerNotificationPreferenceUpdateType target) {
	...
	public static AccountInformationServiceMapper.MaxVoipLineMapper MaxVoipLineMapper() {
		return AccountInformationServiceMapper.MaxVoipLineMapper.getInstance();
	}
	
	public static class MaxVoipLineMapper extends AbstractSchemaMapper<MaxVoipLineType,MaxVoipLineInfo> {
		
		private static MaxVoipLineMapper INSTANCE;

		public MaxVoipLineMapper() {
			super(MaxVoipLineType.class, MaxVoipLineInfo.class);
		}
		
		public static synchronized MaxVoipLineMapper getInstance() {
			if (INSTANCE == null) {
				INSTANCE = new MaxVoipLineMapper();
			}			
			return INSTANCE;
		}
		
		@Override
		protected MaxVoipLineType performSchemaMapping(MaxVoipLineInfo source, MaxVoipLineType target) {
			...
			target.setMaxVoipLineNumber(new Long(source.getMaxVoipLines()));

			return super.performSchemaMapping(source, target);
		}
		
		@Override
		protected MaxVoipLineInfo performDomainMapping(MaxVoipLineType source, MaxVoipLineInfo target) {
			
			target.setBan(Integer.valueOf(source.getBillingAccountNumber()));
			target.setSubscriptionId(source.getSubscriptionId());
			target.setEffectiveStartDate(source.getEffectiveStartDate());
			target.setEffectiveEndDate(source.getEffectiveEndDate());
			target.setMaxVoipLines(source.getMaxVoipLineNumber().intValue());
	...
	public static AccountInformationServiceMapper.MaxVoipLineBaseMapper MaxVoipLineBaseMapper() {
		return AccountInformationServiceMapper.MaxVoipLineBaseMapper.getInstance();
	}
	
	public static class MaxVoipLineBaseMapper extends AbstractSchemaMapper<MaxVoipLineBaseType,MaxVoipLineInfo> {
		
		private static MaxVoipLineBaseMapper INSTANCE;

		public static synchronized MaxVoipLineBaseMapper getInstance() {
			if (INSTANCE == null) {
				INSTANCE = new MaxVoipLineBaseMapper();
			}			
			return INSTANCE;
		}

		public MaxVoipLineBaseMapper() {
			super(MaxVoipLineBaseType.class, MaxVoipLineInfo.class);
		}
	
		@Override
		protected MaxVoipLineBaseType performSchemaMapping(MaxVoipLineInfo source, MaxVoipLineBaseType target) {
		...
		@Override
		protected MaxVoipLineInfo performDomainMapping(MaxVoipLineBaseType source, MaxVoipLineInfo target) {
	...
	public static AccountInformationServiceMapper.PhoneNumberSearchOptionMapper PhoneNumberSearchOptionMapper() {
		return AccountInformationServiceMapper.PhoneNumberSearchOptionMapper.getInstance();
	}	
	
	public static class PhoneNumberSearchOptionMapper extends AbstractSchemaMapper<PhoneNumberSearchOption,PhoneNumberSearchOptionInfo> {
		
		private static PhoneNumberSearchOptionMapper INSTANCE;

		public static synchronized PhoneNumberSearchOptionMapper getInstance() {
			if (INSTANCE == null) {
