...
import com.telus.tmi.xmlschema.xsd.service.basetypes.serviceorderreferencetypes_v5.FundSourceList;
import com.telus.tmi.xmlschema.xsd.service.basetypes.serviceorderreferencetypes_v5.Service;
...
//		target.setCrossFleetRestrictedInd(source.isCrossFleetRestricted());
		...
		target.getNetworkTypeList().addAll(Arrays.asList(source.getAllNetworkTypes()));
		...
		target.setSocServiceType(source.getSocServiceType());
		target.setPpsAddOnInd(source.isPPSAddOn());
		target.setPpsBundleInd(source.isPPSBundle());
		target.setPpsIncludedLicenseSize(source.ppsLicenses());
		target.setPpsPriority(source.ppsPriority());
		target.setPpsStorageSize(source.ppsStorage());
		target.setHasPromotionInd(source.hasPromotion());
		target.setDurationServiceHoursNum(source.getDurationServiceHours());
		target.setRoamLikeHomeInd(source.isRLH());
		target.setFlexPlanInd(source.isFlexPlan());
		target.setMandatoryAddOnInd(source.isMandatoryAddOn());
