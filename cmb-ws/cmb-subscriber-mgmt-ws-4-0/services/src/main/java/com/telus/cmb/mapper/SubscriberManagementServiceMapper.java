...
import com.telus.cmb.jws.CompletePortRequestDataType;
import com.telus.cmb.jws.PortInEligibility;
import com.telus.cmb.jws.PortRequestData;
import com.telus.cmb.jws.ServicesValidation;
import com.telus.cmb.jws.SummaryPortRequestDataType;
...
public class SubscriberManagementServiceMapper {
	...
	public static PortRequestMapper PortRequestMapper() {
		return PortRequestMapper.getInstance();
	}
	...
	public static PortRequestSummaryMapper PortRequestSummaryMapper() {
		return PortRequestSummaryMapper.getInstance();
	}
	
	public static CurrentPortRequestMapper CurrentPortRequestMapper() {
		return CurrentPortRequestMapper.getInstance();
	}
...
//			target.setServiceType(source.getPricePlan0().getServiceType());
	...
	public static class PortRequestMapper extends AbstractSchemaMapper<PortRequestData, PortRequestInfo> {
		private static PortRequestMapper INSTANCE;
		
		private PortRequestMapper() {
			super (PortRequestData.class, PortRequestInfo.class);
		}
		
		protected static PortRequestMapper getInstance() {
			if (INSTANCE == null) {
				INSTANCE = new PortRequestMapper();
			}
			return INSTANCE;
		}
		...
		protected PortRequestInfo performDomainMapping(PortRequestData source, PortRequestInfo target) {
			...
			//target.setAutoActivate(Boolean.parseBoolean(source.getAutoActivateCd())); 
			if(source.getBillingAccountNumber() != null) {
				target.setBanId(Integer.parseInt(source.getBillingAccountNumber()));
			}
			/*target.setCanBeActivate(source.isCanBeActivateInd());
			target.setCanBeCancel(source.isCanBeCanceledInd());
			target.setCanBeModify(source.isCanBeModifiedInd());
			target.setCanBeSubmit(source.isCanBeModifiedInd());
			target.setCreationDate(source.getCreationDate());
			target.setDesiredDateTime(source.getDesiredDueDate());
			...
			/*target.setType(source.getPortProcessType());
			target.setRemarks(source.getRemarksTxt());
			target.setStatusCategory(source.getStatusCategoryCd());
			target.setStatusCode(source.getStatusCd());
			target.setStatusReasonCode(source.getStatusReasonCd());
			target.setType(source.getPortProcessType());
			if(source.getSourceBrandId()!=null){
				target.setIncomingBrandId(Integer.parseInt(source.getSourceBrandId()));;
			}
			if(source.getTargetBrandId()!=null){
				target.setOutgoingBrandId(Integer.parseInt(source.getTargetBrandId()));
			}*/
			if(source.getPortRequestAddress()!=null){
	...
	public static class PortRequestSummaryMapper extends AbstractSchemaMapper<SummaryPortRequestDataType,PortRequestSummary> {
		private static PortRequestSummaryMapper INSTANCE;
		
		private PortRequestSummaryMapper() {
			super (SummaryPortRequestDataType.class, PortRequestSummary.class);
		}
		
		protected static PortRequestSummaryMapper getInstance() {
			if (INSTANCE == null) {
				INSTANCE = new PortRequestSummaryMapper();
			}
			return INSTANCE;
		}
	...
	public static class CurrentPortRequestMapper extends AbstractSchemaMapper<CompletePortRequestDataType, PortRequestInfo> {
		private static CurrentPortRequestMapper INSTANCE;
		
		private CurrentPortRequestMapper() {
			super (CompletePortRequestDataType.class, PortRequestInfo.class);
		}
		
		protected static CurrentPortRequestMapper getInstance() {
			if (INSTANCE == null) {
				INSTANCE = new CurrentPortRequestMapper();
			}
			return INSTANCE;
		}
