...
import com.telus.cmb.jws.mapping.WirelessAccountLifecycleMgmtMapper.BusinessRegistrationMapper;
...
@WebService(portName = "WirelessAccountLifecycleMgmtServicePort", serviceName = "WirelessAccountLifecycleMgmtService_v1_4", targetNamespace = "http://telus.com/wsdl/CMO/OrderMgmt/WirelessAccountLifecycleMgmtService_1", wsdlLocation = "/wsdls/WirelessAccountLifecycleMgmtService_v1_4.wsdl", endpointInterface = "com.telus.cmb.jws.WirelessAccountLifecycleMgmtServicePort")
@BindingType("http://schemas.xmlsoap.org/wsdl/soap/http")
@SchemaValidation(handler = com.telus.cmb.jws.ServiceSchemaValidator.class)
public class WirelessAccountLifecycleMgmtService_14 extends BaseService implements WirelessAccountLifecycleMgmtServicePort {
	...
	public CreatePostpaidAccountResponse createPostpaidAccount(final CreatePostpaidAccount parameters, final OriginatingUserType createPostpaidAccountInSoapHdr) throws PolicyException,
		...
		return execute(new ServiceInvocationCallback<CreatePostpaidAccountResponse>() {
			...
			public CreatePostpaidAccountResponse doInInvocationCallback(ServiceInvocationContext context) throws Throwable {
				...
				PostpaidAccountRequest request = parameters.getBaseAccountRequest();
				...
				AccountInfo accountInfo = getBaseWirelessAccountMapper().mapToDomain(request.getBaseAccount());

				BusinessCreditIdentityInfo[] creditIdentityInfos = null;
				if (request.getBusinessCreditIdentityList() != null && request.getBusinessCreditIdentityList().getItem().size() > 0) {
					List<BusinessCreditIdentityInfo> businessCreditIdentityList = new ArrayList<BusinessCreditIdentityInfo>();

					List<BusinessCreditIdentity> sourceList = request.getBusinessCreditIdentityList().getItem();
				...
				BusinessCreditIdentityInfo selectedBusinessCreditIdentity = getBusinessCreditIdentityMapper().mapToDomain(request.getSelectedBusinessCreditIdentity());
				AuditInfo auditInfo = getAuditInfoMapper().mapToDomain(request.getAuditInfo());

				setTelusTidToMessageContext(auditInfo);
				setBusinessRegistrationInfo(request, accountInfo);
				setForceZeroBalanceIndicator(request, accountInfo);

				String customerId = null;
				if (request.getCompassCustomerId() != null) {
					customerId = request.getCompassCustomerId().toString();
				}
	...
	private void setBusinessRegistrationInfo(PostpaidAccountRequest request, AccountInfo account) {		
		if (account.isPostpaidBusinessPersonal()) {			
			((PostpaidBusinessPersonalAccountInfo)account).setBusinessRegistration(getBusinessRegistrationMapper().mapToDomain(request.getBusinessRegistrationInfo()));	
		}
	}
	
	private void setForceZeroBalanceIndicator(PostpaidAccountRequest request, AccountInfo account) {		
		if (isHedwigInternalAccount(account)) {
			account.setForceZeroBalanceInd(request.isForceZeroBalanceInd() == null ? false : request.isForceZeroBalanceInd());	
		}
	}
	
	private boolean isHedwigInternalAccount(AccountInfo account) {
		return account.isPostpaidBusinessOfficial()
				|| (account.isCorporate() && (account.getAccountSubType() == AccountSummary.ACCOUNT_SUBTYPE_CORP_PCS_TMI_AFFILIATE || account.getAccountSubType() == AccountSummary.ACCOUNT_SUBTYPE_CORP_PCS_CORPORATE));
	}
	...
	private BusinessRegistrationMapper getBusinessRegistrationMapper() {
		return WirelessAccountLifecycleMgmtMapper.BusinessRegistrationMapper();
	}
