...
import com.telus.cmb.schema.ChangePhoneNumberResponseType;
 ...
 *
 */
@WebService(portName = "SubscriberManagementServicePort", serviceName = "SubscriberManagementService_v4_1", targetNamespace = "http://telus.com/wsdl/CMO/OrderMgmt/SubscriberManagementService_4", wsdlLocation = "/wsdls/SubscriberManagementService_v4_1.wsdl", endpointInterface = "com.telus.cmb.endpoint.SubscriberManagementServicePort")
...
public class SubscriberManagementService extends EndpointProvider implements SubscriberManagementServicePort {
	...
	public ChangeEquipmentResponse changeEquipment(ChangeEquipment params) throws EndpointException {
		...
		String sessionId = getSessionId(params.getSessionIdentifier(), subscriberFacade);
	...
	public ChangePhoneNumberResponse changePhoneNumber(ChangePhoneNumber params) throws EndpointException {
		...
		phoneNumberChangeInfo.setDealerCode(profile.getDealerCode());
		phoneNumberChangeInfo.setSalesRepCode(profile.getSalesRepCode());
		phoneNumberChangeInfo.setSubscriberId(subscriberData.getSubscriberId());

		PhoneNumberChangeRequest numberChangeRequest = params.getPhoneNumberChangeRequest();
		if (numberChangeRequest.isChangeOtherNumbersInd() != null) {
			phoneNumberChangeInfo.setChangeOtherPhoneNumbers(numberChangeRequest.isChangeOtherNumbersInd().booleanValue());
		}
		phoneNumberChangeInfo.setNewPhoneNumber(numberChangeRequest.getPhoneNumber().getPhoneNumber());
		phoneNumberChangeInfo.setReasonCode(numberChangeRequest.getReasonCode());

		PhoneNumberChangeInfo newPhoneNumberChangeInfo = null;

		if (subscriberData.getAccount() != null && subscriberData.getSubscriber() != null) {
			newPhoneNumberChangeInfo = subscriberFacade.changePhoneNumber(getSubscriberMapper().mapToDomain(subscriberData.getSubscriber()), getAccountMapper().mapToDomain(subscriberData.getAccount()), phoneNumberChangeInfo, sessionId);
		} else {
			newPhoneNumberChangeInfo = subscriberFacade.changePhoneNumber(subscriberData.getSubscriberId(), Integer.valueOf(subscriberData.getBillingAccountNumber()), phoneNumberChangeInfo, sessionId);
		}

		ChangePhoneNumberResponseType responseType = new ChangePhoneNumberResponseType();
		...
		try {
			...
			subscriberFacade.reportChangePhoneNumber(Integer.parseInt(subscriberData.getBillingAccountNumber()), newPhoneNumberChangeInfo, servReqHeaderInfo, sessionId);
		} catch (ApplicationException ae) {
			WarningFaultInfo warning = new WarningFaultInfo(WarningFaultInfo.APPLICATION_EXCEPTION, ae.getSystemCode(), null, ae.getErrorCode(), ae.getErrorMessage(), null);
			responseType.getWarningList().add(getServiceWarningMapper().mapToSchema(warning));
		} catch (SystemException se) {
		...
		responseType.getWarningList().addAll(getServiceWarningMapper().mapToSchema(newPhoneNumberChangeInfo.getApplicationWarningList()));
	...
	public ValidateServiceAgreementResponse validateServiceAgreement(ValidateServiceAgreement parameters) throws EndpointException {
		...
		String sessionId = getSessionId(parameters.getSessionIdentifier(), subscriberFacade);
	...
	public ValidateMigrateSubscriberResponse validateMigrateSubscriber(ValidateMigrateSubscriber parameters) throws EndpointException {
		...
		migrationChangeInfo = subscriberFacade.validateMigrateSubscriber(migrationChangeInfo, getSessionId(subscriberFacade));
	...
	public MigrateSubscriberResponse migrateSubscriber(MigrateSubscriber parameters) throws EndpointException {
		...
		changeInfo = subscriberFacade.migrateSubscriber(changeInfo, getSessionId(subscriberFacade));
	...
	private MigrationChangeInfo mapToMigrationChangeInfo(UserServiceProfile userProfile, SubscriberData subscribertData, String migrationTypeCode, 
			String pricePlanCode, Equipment newEquipment, Equipment newAssociatedHandset, String requestorId) {
	...
	public ActivateSubscriberResponse activateSubscriber(ActivateSubscriber parameters) throws EndpointException {
		...
		ActivationChangeInfo changeInfo = mapToActivationChangeInfo(parameters.getUserProfile(), parameters.getActivationOption(), parameters.isDealerHasDepositInd(), 
				...
				parameters.getEquipmentRequest(), parameters.getSubscriptionRoleCd());

		changeInfo = subscriberFacade.activateSubscriber(changeInfo, getSessionId(subscriberFacade));
	...
	private ActivationChangeInfo mapToActivationChangeInfo(UserServiceProfile userProfile, ActivationOption activationOption, Boolean dealerHasDepositInd, String activityReasonCode, 
			...
			ServicesValidation servicesValidation, ServiceAgreement serviceAgreement, EquipmentActivated equipmentRequest, String subscriptionRoleCd) throws TelusAPIException {
	...
	public ValidateMigrateSeatResponse validateMigrateSeat(ValidateMigrateSeat parameters) throws EndpointException {
		...
		migrateSeatChangeInfo = subscriberFacade.validateMigrateSeat(migrateSeatChangeInfo, parameters.isNotificationSuppressionInd(), 
				auditInfo, getSessionId(subscriberFacade));
	...
	public ChangeVoipNumberResponse changeVoipNumber(ChangeVoipNumber parameters) throws EndpointException {
		...
		// Make the call to the SubscriberLifecycleFacade EJB
		subscriberFacade.changeVOIPResource(parameters.getSubscriptionId(), resourceList, null, parameters.isOutgoingRequestInd(), getSessionId(subscriberFacade));
	...
	public ChangeVoipNumberWithChargeResponse changeVoipNumberWithCharge(ChangeVoipNumberWithCharge parameters) throws EndpointException {
		...
		// Make the call to the SubscriberLifecycleFacade EJB
		subscriberFacade.changeVOIPResourceWithCharge(parameters.getSubscriptionId(), resourceActivity, null, parameters.isOutgoingRequestInd(), parameters.getRegularServiceCd(), getSessionId(subscriberFacade));
	...
	public MoveVoipNumberBetweenSeatsResponse moveVoipNumberBetweenSeats(MoveVoipNumberBetweenSeats parameters) throws EndpointException {
		...
		subscriberFacade.moveVOIPResource(parameters.getSourceSubscriptionId(), parameters.getTargetSubscriptionId(), 
				parameters.getSeatResource().getSeatResourceNumber(), null, outgoingRequestInd, getSessionId(subscriberFacade));
	...
	public ResetCscSubscriptionResponse resetCscSubscription(ResetCscSubscription parameters) throws EndpointException {
		...
		subscriberManager.resetCSCSubscription(ban, subscriberId, productType, getSessionId(subscriberFacade));
	...
	public ActivatePortInRequestResponse activatePortInRequest(ActivatePortInRequest parameters) throws EndpointException {
		...
		ActivationChangeInfo changeInfo = subscriberFacade.performPortInActivation(getSessionId(subscriberFacade),
