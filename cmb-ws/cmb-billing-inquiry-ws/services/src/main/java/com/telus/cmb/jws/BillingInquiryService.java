...
import com.telus.cmb.jws.mapper.BillingInquiryMapper_15;
...
@WebService(portName = "BillingInquiryServicePort", 
			serviceName = "BillingInquiryService_v1_7", 
			targetNamespace = "http://telus.com/wsdl/CMO/BillingInquiryMgmt/BillingInquiryService_1", 
			wsdlLocation = "/wsdls/BillingInquiryService_v1_7.wsdl", 
			endpointInterface = "com.telus.cmb.jws.BillingInquiryServicePort")

@BindingType("http://schemas.xmlsoap.org/wsdl/soap/http")

public class BillingInquiryService extends BaseService implements BillingInquiryServicePort {
	...
	public ResponseMessage applyCreditToAccountForSubscriber(final String accountNumber, final String subscriberNumber, final String reasonCode, final String memoText, final Date effectiveDate, final double amount,
		...
		return execute(new ServiceInvocationCallback<ResponseMessage>() {
			...
			public ResponseMessage doInInvocationCallback(ServiceInvocationContext context) throws Throwable {
				...
				creditInfo.setPhoneNumber(subscriberNumber);
...
// Start BISv1.5	
	@ServiceBusinessOperation(errorCode = "CMB_BIS_0042", errorMessage = "Process Payment error")
	public ProcessPaymentResponse processPayment(final ProcessPayment parameters, final OriginatingUserType inputSoapHeader) throws PolicyException, ServiceException {
		...
		return execute(new ServiceInvocationCallback <ProcessPaymentResponse>() {
			...
			public ProcessPaymentResponse doInInvocationCallback(ServiceInvocationContext context) throws Throwable {
				...
				int ban = Integer.parseInt(parameters.getCreditCardTransaction().getBanId());
				double amount = Double.parseDouble(parameters.getCreditCardTransaction().getAmount());

				// CC validation and mapping
				CreditCardTransaction creditCardTransaction = parameters.getCreditCardTransaction();
				TransactionType transactionType = creditCardTransaction.getTransactionType();
				...
				if (transactionType != TransactionType.CCP) {
					throw new ServicePolicyException(ServiceErrorCodes.ERROR_INVALID_CREDIT_CARD_TRANSACTION_TYPE, "Credit card transaction type [" + transactionType + "] is not supported");
				}

				CreditCardTransactionInfo creditCardTransactionInfo =  BillingInquiryMapper_15.CreditCardTransactionMapper().mapToDomain(creditCardTransaction);
				...
				String authNumber = getAccountLifecycleFacade(context).processPayment(amount, accountInfo, creditCardTransactionInfo, paymentSourceType, paymentSourceID,
						parameters.isNotificationSuppressionInd(), auditInfo, sessionId);
				response.setAuthorizationNumber(authNumber);
...
// Start BISv1.6
	...
	@ServiceBusinessOperation(errorCode = "CMB_BIS_0043", errorMessage = "Process deposit error")
	public ProcessDepositResponse processDeposit(final ProcessDeposit parameters, final OriginatingUserType inputSoapHeader) throws PolicyException, ServiceException {
		...
		return execute(new ServiceInvocationCallback <ProcessDepositResponse>() {
			...
			public ProcessDepositResponse doInInvocationCallback(ServiceInvocationContext context) throws Throwable {
				...
				String authNumber = getAccountLifecycleFacade(context).payDeposit(amount, creditCardTransactionInfo, ban, paymentSourceType, paymentSourceID, context.getAccountLifeCycleFacadeSessionId());				
				response.setAuthorizationNumber(authNumber);
...
// End BISv1.6
	...
	@ServiceBusinessOperation(errorCode = "CMB_BIS_0044", errorMessage = "Apply ChargesAndAdjustmentsWithTax To Account error")
	public List<ChargesAndAdjustmentsId> applyChargesAndAdjustmentsToAccountWithTax(final List<ChargeAndAdjustment> chargesAndAdjustmentsList, final ProvinceCode taxationProvinceCode, 
			final Boolean waiveTaxInd, final Boolean notificationSuppressionInd, final AuditInfo auditInfo) throws PolicyException, ServiceException {
		throw new ServicePolicyException(ServiceErrorCodes.ERROR_UNSUPPORTED_OPERATION, "This operation is not yet implemented.");
		/* Uncomment the code below when operation is ready to be used by consumers
		return execute(new ServiceInvocationCallback<List<ChargesAndAdjustmentsId>>() {
			@Override
			public List<ChargesAndAdjustmentsId> doInInvocationCallback(ServiceInvocationContext context) throws Throwable {				
				List<ChargeAdjustmentInfo> list = BillingInquiryMapper.ChargesAndAdjustmentMapper().mapToDomain(chargesAndAdjustmentsList);
				...
				List<ChargeAdjustmentInfo> resultList = getAccountLifecycleManager(context).applyChargesAndAdjustmentsToAccountWithTax(list, taxationProvinceCode.value(), 
						(waiveTaxInd == null) ? false : waiveTaxInd.booleanValue(), context.getAccountLifeCycleManagerSessionId());				
				return BillingInquiryMapper.ChargesAndAdjustmentIdMapper().mapToSchema(resultList);
			}
		});
		*/
	...
	@ServiceBusinessOperation(errorCode = "CMB_BIS_0045", errorMessage = "Apply ChargesAndAdjustmentsWithTax To AccountForSubscriber error")
	public List<ChargesAndAdjustmentsId> applyChargesAndAdjustmentsToAccountForSubscriberWithTax(final List<ChargeAndAdjustmentForSubscriber> chargesAndAdjustmentsList, 
			final ProvinceCode taxationProvinceCode, final Boolean waiveTaxInd, final Boolean notificationSuppressionInd, final AuditInfo auditInfo) throws PolicyException, ServiceException {
		
		return execute(new ServiceInvocationCallback<List<ChargesAndAdjustmentsId>>() {
			@Override
			public List<ChargesAndAdjustmentsId> doInInvocationCallback(ServiceInvocationContext context) throws Throwable {
				List<ChargeAdjustmentInfo> list = BillingInquiryMapper.ChargesAndAdjustmentToSubscriberMapper().mapToDomain(chargesAndAdjustmentsList);
				if (isBypassAuthorization(list)) {
					setUseApplicationIdentity();
				}
				List<ChargeAdjustmentInfo> resultList = getAccountLifecycleManager(context).applyChargesAndAdjustmentsToAccountForSubscriberWithTax(list, taxationProvinceCode.value(),	
						(waiveTaxInd == null) ? false : waiveTaxInd.booleanValue(), context.getAccountLifeCycleManagerSessionId());				
				return BillingInquiryMapper.ChargesAndAdjustmentIdMapper().mapToSchema(resultList);
			}
		});
