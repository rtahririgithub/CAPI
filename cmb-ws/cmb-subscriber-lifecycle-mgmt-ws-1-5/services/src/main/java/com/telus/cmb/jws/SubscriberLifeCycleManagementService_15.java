...
@WebService(
		...
		serviceName = "SubscriberLifeCycleManagementService_v1_5", 
		targetNamespace = "http://telus.com/wsdl/CMO/OrderMgmt/SubscriberLifeCycleManagementService_1", 
		wsdlLocation = "/wsdls/SubscriberLifeCycleManagementService_v1_5.wsdl", 
		endpointInterface = "com.telus.cmb.jws.SubscriberLifeCycleManagementServicePort")
		
@BindingType("http://schemas.xmlsoap.org/wsdl/soap/http")

public class SubscriberLifeCycleManagementService_15 extends BaseService implements SubscriberLifeCycleManagementServicePort{
						...
						switch (accountStatus) {
							...
							default:
								context.getAccountLifecycleFacade().suspendSubscribers(ban, suspendPropertiesType.getActivityDate(), suspendPropertiesType.getReasonCode(),
										subscriberIdList.toArray(new String[subscriberIdList.size()]), suspendPropertiesType.getMemoText(), context.getAccountLifeCycleFacadeSessionId());
						}
					}
	...
	@ServiceBusinessOperation(errorCode = "CMB_SLCMS_0014", errorMessage = "validateDataSharingBeforeCancelSubscriber Error")
	public void validateDataSharingBeforeCancelSubscriber(final String subscriberId, Holder<ResponseMessage> responseMessage, 
			Holder<Boolean> dataSharingViolationInd, Holder<DataSharingSocTransfer> dataSharingSocTransfer)	throws PolicyException, ServiceException {
		
		ValidateDataSharingBeforeCancelSubscriberResponse response = execute(new ServiceInvocationCallback<ValidateDataSharingBeforeCancelSubscriberResponse>() {
			@Override
			public ValidateDataSharingBeforeCancelSubscriberResponse doInInvocationCallback(ServiceInvocationContext context) throws Throwable {

				ValidateDataSharingBeforeCancelSubscriberResponse response = new ValidateDataSharingBeforeCancelSubscriberResponse();
				ResponseMessage responseMessage = new ResponseMessage();
				try {
					DataSharingSocTransferInfo dsSocTransferInfo = context.getSubscriberLifecycleFacade().validateDataSharingBeforeCancelSubscriber(subscriberId);
					if (dsSocTransferInfo == null) {
						response.setDataSharingViolationInd(false);	
					} else {
						response.setDataSharingViolationInd(true);
						response.setDataSharingSocTransfer(getDataSharingSocTransferMapper().mapToSchema(dsSocTransferInfo));
					}
					responseMessage.setDateTimeStamp(new Date());					
				} catch (ApplicationException ae) {
					responseMessage.setErrorCode(ae.getErrorCode());
		...
		dataSharingViolationInd.value = response.isDataSharingViolationInd();
		dataSharingSocTransfer.value = response.getDataSharingSocTransfer();
		responseMessage.value = response.getResponseMessage();
	}
	...
	private SubscriberLifeCycleManagementServiceMapper.DataSharingSocTransferMapper getDataSharingSocTransferMapper() {
		return SubscriberLifeCycleManagementServiceMapper.DataSharingSocTransferMapper();
	}
