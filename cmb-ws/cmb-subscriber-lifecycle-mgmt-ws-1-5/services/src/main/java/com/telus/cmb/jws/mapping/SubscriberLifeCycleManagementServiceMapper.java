...
import com.telus.cmb.jws.DataSharingSocTransfer;
...
public class SubscriberLifeCycleManagementServiceMapper {
	...
	public static DataSharingSocTransferMapper DataSharingSocTransferMapper() {
		return DataSharingSocTransferMapper.getInstance();
	}	
	...
	public static class DataSharingSocTransferMapper extends AbstractSchemaMapper<DataSharingSocTransfer, DataSharingSocTransferInfo> {
		private static DataSharingSocTransferMapper INSTANCE;
		
		private DataSharingSocTransferMapper() {
			super(DataSharingSocTransfer.class, DataSharingSocTransferInfo.class);
		}
		
		protected static DataSharingSocTransferMapper getInstance() {
			if (INSTANCE == null) {
				INSTANCE = new DataSharingSocTransferMapper();
			}
			return INSTANCE;
		}
		
		@Override
		protected DataSharingSocTransfer performSchemaMapping(DataSharingSocTransferInfo source, DataSharingSocTransfer target) {
			target.setCandidateSubscriberId(source.getCandidateSubscriberId());						
			List<Description> candidateSocDescription = Arrays.asList(
					createDescription("EN_CA", source.getCandidateSocDescription()), 
					createDescription("FR_CA", source.getCandidateSocDescriptionFrench())); 
			target.setCandidateDataSharingSoc(mapCodeDescriptionList(source.getCandidateSocCode(), candidateSocDescription));
			List<Description> contributorSocDescription = Arrays.asList(
					createDescription("EN_CA", source.getContributorSocDescription()), 
					createDescription("FR_CA", source.getContributorSocDescriptionFrench())); 
			target.setContributorDataSharingSoc(mapCodeDescriptionList(source.getContributorSocCode(), contributorSocDescription));
			return super.performSchemaMapping(source, target);
		}
		
		private MultilingualCodeDescriptionList mapCodeDescriptionList(String code, List<Description> descriptionList) {
			MultilingualCodeDescriptionList codeDescriptionList = new MultilingualCodeDescriptionList();
			codeDescriptionList.setCode(code);
			codeDescriptionList.setDescription(descriptionList);
			return codeDescriptionList;
		}
		
		private Description createDescription(String locale, String descriptionText) {
