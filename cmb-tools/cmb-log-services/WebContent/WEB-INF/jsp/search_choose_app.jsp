...
<html>
	...
	<body id='logSearch'>
		...
		<div class='ui large attached message'>
			<div class='header'>
				Select an application deployed in <a href="${searchURL}">${environment.name}</a>.  
			</div>
		</div>
					...
					<div class='ui fluid green selection dropdown'>
						...
						<div class='menu'>				
							<c:forEach items="${applications}" var="application" varStatus="status">
								<a href="${searchURL}/${environment.shortname}/${application.shortname}" class='item'>${application.name}</a>
							</c:forEach>					
						</div>
					</div>
				</div>
