...
abstract class TransitionMatrix {
  ...
  /**
   * The state rather this object was created with all data loaded successfully.
   */
  ...
  abstract void initialize() throws Throwable;
	...
	synchronized void retryInitialization() {
		try {
			initialize();
		} catch (Throwable t) {
			throw new SystemException(SystemCodes.CMB_SLF_EJB, "Error in initialize()", "", t);
		}
	}
  ...
  final void addInitState(State newInitState) {
	...
	protected boolean isInitializedSuccessfully() {
		return initializedSuccessfully;
	}

	protected void setInitializedSuccessfully(boolean initializedSuccessfully) {
...
/**
  ...
  final ValidationResult validTransition(State initState, State[] finalStates, boolean matchAll) {
	...
	protected ReferenceDataHelper getReferenceDataHelper() throws ApplicationException {
		return EJBUtil.getHelperProxy(ReferenceDataHelper.class, EJBUtil.TELUS_CMBSERVICE_REFERENCE_DATA_HELPER);
	}
}
