...
-- Sep 16,2010    Michael Liao       - Modify Query in following procedures to return new column BILL_CYCLE_TREATMENT_CD on SOC table 
...
-- 
-- Jul 12,2012 (3.20.1) Michael Liao   - Modify query in getfeaturesforservice  , getfeaturesforpriceplan: 
--                                     - add criteria: PP_UC_RATE.RATE_VERSION_NUM=0 
-- Jul 18,2012 (3.20.2) Michael Liao   - bug fix: query in getfeaturesforservice  , getfeaturesforpriceplan shall use outter join for PP_UC_RATE.RATE_VERSION_NUM
-- Oct 25,2013	        Kris Viceral   - bug fix: getpriceplanlist: Province availability issue: Plans not available for any Province when db entry is updated with "ALL"
...
-- Jul 14,2014 (3.29.1) Maxim Strigachov - Added support for duration service addition to a contract.
-- Jul 16,2014 (3.29)   Maxim Strigachov - Modified the following functions/procedures for RLH:
...
-- Feb 17,2015 (3.40)   Richard Fong   - Modified the following functions/procedures for RLH phase 2:
...
-- Mar 28,2019 (3.57)   Wilson Cheong  - Modified the following functions/procedures for Tallboy:
-- 									   - getpriceplan: added combo_group_id
--                                     - Note: getpriceplanlist is deprecated in the code (no changes necessary)
-------------------------------------------------------------------------------------------------------------------------------------
...
-- error messages
   ...
   version_no          CONSTANT VARCHAR2(10)       := '3.57';
   ...
   IS
      ...
      IS
         SELECT DISTINCT s.service_type, s.soc, s.soc_description,
						 ...
						 s.soc_duration_hours, s.rlh_ind, s.combo_group_id
                    FROM soc s,
                         soc_group sg,
                         priceplan_soc_groups psg
...
*/      
   ...
   BEGIN
	  ...
	  v_service_feature_ob.soc_duration_hours := services_rec.soc_duration_hours;
	  v_service_feature_ob.rlh_ind := services_rec.rlh_ind;
	  v_service_feature_ob.combo_group_id := services_rec.combo_group_id;
