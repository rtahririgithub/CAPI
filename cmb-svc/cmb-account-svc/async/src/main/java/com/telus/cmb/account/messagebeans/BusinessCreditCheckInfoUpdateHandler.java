...
public class BusinessCreditCheckInfoUpdateHandler implements MessageHandlerList<List<Object>>{
	...
	public void handle(List<Object> messageList, ClientIdentity clientIdentity,
		...
		accountLifecycleManager.saveCreditCheckInfoForBusiness(
				(AccountInfo)messageList.get(1), 
				(BusinessCreditIdentityInfo[]) messageList.get(2),
				(BusinessCreditIdentityInfo) messageList.get(3),
				(CreditCheckResultInfo)messageList.get(4), 
				(String)messageList.get(5), sessionId);
	}
