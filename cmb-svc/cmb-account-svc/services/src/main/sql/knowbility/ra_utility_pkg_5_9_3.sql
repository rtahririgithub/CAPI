...
End GetAccountInfoByBAN;
	...
	BEGIN
		...
		WHILE i_Length > 255 LOOP
			DBMS_OUTPUT.PUT_LINE(SUBSTR(v_Line, i_Index, 255));					
			i_Index := i_Index + 255;
			i_Length := i_Length - 255;
		END LOOP;
		
		DBMS_OUTPUT.PUT_LINE(SUBSTR(v_Line, i_Index, LENGTH(v_Line) - i_Index + 1));
	END PrintLn;
	...
	BEGIN
		BEGIN
			IF i_BAN_Numbers.COUNT > 0 THEN
    			v_Cursor_Text := 'SELECT DISTINCT ba.ban, '
                	          || '       ba.ban_status, '
                    	      || '       ba.account_type, '
                      		  || '       ba.account_sub_type, '
        		...
        		WHILE i_Index < i_BAN_Numbers.COUNT LOOP
    			...
    			OPEN o_Accounts FOR v_Cursor_Text;
	...
	BEGIN                            
		BEGIN
			b_Full_Join := (TO_CHAR(c_Account_Status) != SEARCH_ALL) 
			...
			IF b_Full_Join THEN
				v_Cursor_Text := v_Cursor_Text || '  FROM name_data nd, address_name_link anl, billing_account ba ';
			ELSE
				v_Cursor_Text := v_Cursor_Text || '  FROM name_data nd, address_name_link anl ';
			END IF;			

			IF i_Control_Name_Exact = NUMERIC_TRUE THEN
				v_Cursor_Text := v_Cursor_Text || ' WHERE nd.control_name = ''' || UPPER(v_Control_Name) || ''' ';
			ELSE
				v_Cursor_Text := v_Cursor_Text || ' WHERE nd.control_name LIKE ''' || UPPER(v_Control_Name) || '%'' ';
			END IF;

			IF LENGTH(RTRIM(LTRIM(v_First_Name))) > 0 THEN
				IF i_First_Name_Exact = NUMERIC_TRUE THEN
					v_Cursor_Text := v_Cursor_Text || '   AND nd.first_name = ''' || UPPER(v_First_Name) || ''' ';
				ELSE
					v_Cursor_Text := v_Cursor_Text || '   AND nd.first_name LIKE ''' || UPPER(v_First_Name) || '%'' ';
				END IF;
			END IF;
			...
			IF TO_CHAR(c_Name_Type) != SEARCH_ALL THEN
				v_Cursor_Text := v_Cursor_Text || '   AND anl.link_type = ''' || TO_CHAR(c_Name_Type) || ''' ';
			END IF;
			
			IF b_Full_Join THEN
				v_Cursor_Text := v_Cursor_Text || '   AND ba.ban = anl.customer_id ';
				
				IF TO_CHAR(c_Account_Type) != SEARCH_ALL THEN
					v_Cursor_Text := v_Cursor_Text || '   AND ba.account_type = ''' || c_Account_Type || ''' ';
				END IF;
				
				IF TO_CHAR(c_Account_Status) != SEARCH_ALL THEN
					v_Cursor_Text := v_Cursor_Text || '   AND ba.ban_status = ''' || c_Account_Status || ''' ';
				END IF;
				
				IF TO_CHAR(v_Province) != SEARCH_ALL THEN
					v_Cursor_Text := v_Cursor_Text || '   AND ba.home_province = ''' || v_Province || ''' ';
				END IF;
			END IF;
			...
			OPEN c_Account_Cursor FOR v_Cursor_Text;
			...
			IF i_Count > i_Max AND NOT c_Account_Cursor%NOTFOUND THEN
			...
			IF i_Count > 1 THEN
	        	WHILE a_BAN_Array(a_BAN_Array.COUNT) IS NULL LOOP
    	        	a_BAN_Array.TRIM;
	    	    END LOOP;

				IF i_Has_More = NUMERIC_TRUE THEN
					a_BAN_Array.TRIM;
				END IF;

				i_Result := GetAccountsByBANs(a_BAN_Array, o_Accounts, v_Error_Message);
			ELSE
				v_Error_Message := ERR_NO_DATA_FOUND;	
				i_Result := NUMERIC_FALSE;
			END IF;	
