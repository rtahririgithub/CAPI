...
-- absolute maximum for the number of accounts to be retrieved
   ...
   version_no          CONSTANT VARCHAR2(10)       := '3.23.7';

   FUNCTION getbilledcredits (
      i_ban             IN       NUMBER,
      o_from_date       IN       DATE,
      o_to_date         IN       DATE,
...
AS
   ...
   BEGIN
      BEGIN
         ...
         v_stmt :=
            ...
            || ' AND     a.adj_creation_date between '''
            || o_from_date
            || ''' and '''
            || o_to_date
            || ''''
         ...
         THEN
            v_stmt_sub_clause :=
                       ' AND a.subscriber_no = ''' || v_subscriber_no || '''';
         ELSIF i_is_sub_level = numeric_true
               AND i_is_ban_level = numeric_false
         THEN
         ...
         v_stmt := v_stmt || ' AND rownum <= ' || i_max;
         v_stmt :=
                v_stmt || ' ORDER BY adj_creation_date desc, ent_seq_no desc ';
   ...
   BEGIN
      BEGIN
         ...
         IF v_reason_code IS NOT NULL
         THEN
            v_stmt := v_stmt || ' AND actv_reason_code = ''' || v_reason_code || '''';
            v_stmt_2 := v_stmt_2 || ' AND actv_reason_code = ''' || v_reason_code || '''';
            v_stmt_3 := v_stmt_3 || ' AND a.actv_reason_code = ''' || v_reason_code || '''';
         END IF;
   ...
   BEGIN
      BEGIN
         ...
         v_stmt :=
            ...
            || ' WHERE    c.ban = '
            || i_ban
            || ' AND      c.chg_creation_date between '''
            || o_from_date
            || ''' and '''
            || o_to_date
            || ''''
         ...
         v_stmt :=
            ...
            || ' tax_roaming_amt, tax_gst_exmp_src, tax_pst_exmp_src, tax_hst_exmp_src, tax_roam_exmp_src '
            || ' FROM     pending_charge '
            || ' WHERE ban = '
            || i_ban
            || ' AND approval_status = ''P'' '
         ...
         v_stmt :=
            ...
            || '               c.tax_roaming_amt, c.tax_gst_exmp_src, c.tax_pst_exmp_src, c.tax_hst_exmp_src, c.tax_roam_exmp_src '
            || ' FROM			charge c, bill b '
            || ' WHERE    c.ban = '
            || i_ban
            || ' AND      c.chg_creation_date between '''
