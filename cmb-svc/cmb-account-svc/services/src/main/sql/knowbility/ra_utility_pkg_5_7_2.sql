...
---------------------------------------------------------------------------
-- description: Procedure GetAccountsByBANs returns array of AccountSummary
--              for an array of BANs.
---------------------------------------------------------------------------
PROCEDURE GetAccountsByBANs(i_BAN_Numbers        IN      T_BAN_ARRAY,
    ...
    END IF;
     ...
     Exception
	   ...
	   If ( o_Accounts%ISOPEN ) Then
	    close o_Accounts;
    	End If;
	   Raise_Application_Error(-20160, 'GetAccountsByBANs Query Failed. Oracle:(['
    							|| sqlcode || '] [' || sqlerrm  || '])'  );    
