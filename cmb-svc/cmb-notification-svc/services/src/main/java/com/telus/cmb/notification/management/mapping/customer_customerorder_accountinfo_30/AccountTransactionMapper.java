package com.telus.cmb.notification.management.mapping.customer_customerorder_accountinfo_30;
...
import com.telus.cmb.common.confirmationnotification.MarketingInfo;
...
import com.telus.tmi.xmlschema.xsd.customer.customer.accountinfo_v3.AccountInfo;
import com.telus.tmi.xmlschema.xsd.customer.customer.accountinfo_v3.CreditInfo;
import com.telus.tmi.xmlschema.xsd.customer.customer.accountinfo_v3.CustomerAccount;
import com.telus.tmi.xmlschema.xsd.customer.customer.accountinfo_v3.MarketingOfferInfo;
import com.telus.tmi.xmlschema.xsd.customer.customer.accountinfo_v3.PaymentInfo;
import com.telus.tmi.xmlschema.xsd.customer.customer.accountinfo_v3.PaymentMethodInfo;
import com.telus.tmi.xmlschema.xsd.customer.customer.accountinfo_v3.PaymentReportInfo;
import com.telus.tmi.xmlschema.xsd.customer.customer.accountinfo_v3.DebtSummaryInfo;
...
public class AccountTransactionMapper extends AbstractSchemaMapper<CustomerAccount, ConfirmationNotification> implements AccountTransactionEmailTemplateMapper {
	...
	protected CustomerAccount performSchemaMapping(ConfirmationNotification source, CustomerAccount target) {
		...
		if (source.getMarketingInfo() != null) {
			target.getAccountInfo().setMarketingInd(true);
			target.setMarketingOfferInfo(mapToMarketingOfferInfo(source.getMarketingInfo()));
		}
	...
	private PaymentReportInfo mapToPaymentReportInfo(PaymentNotification source) {
		...
		if(source.getDebtSummary()!=null){
			target.setDebtSummary(mapToDebtSummarySchema(source.getDebtSummary()));
		}
		return target;
	}
	...
	private MarketingOfferInfo mapToMarketingOfferInfo(MarketingInfo source) {
		if (source != null) {
			MarketingOfferInfo target = new MarketingOfferInfo();
			target.setDescriptionEN(source.getMarketingTextEn());
			target.setDescriptionFR(source.getMarketingTextFr());
			target.setOfferCd(source.getOfferCode());
			target.setOfferNameEN(source.getOfferDescriptionEn());
			target.setOfferNameFR(source.getOfferDescriptionFr());
			target.setPromoCd(source.getPromoCode());
			target.setRank(source.getRank());
			target.setValidationCode(source.getValidationCode());
	...
	private DebtSummaryInfo mapToDebtSummarySchema(com.telus.cmb.common.confirmationnotification.DebtSummaryInfo source) {
		DebtSummaryInfo target = new DebtSummaryInfo();
		target.setCurrentDue(source.getCurrentDue());
		target.setPastDue(source.getPastDue());
		target.setAmountDue(source.getCurrentDue() + source.getPastDue()); 
